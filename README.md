...
- 한도 막대·감소 꺾은선 그래프를 7초마다 함께 페이드 전환: 최근 7일은 100%, 하루는 자정 잔여 한도 기준
...
기본 갱신 간격은 5분입니다. 패널을 열 때 데이터가 1분 이상 지났으면 갱신합니다. 창이 보이지 않으면 한도 조회와 이력 저장을 계속하고 서버 토큰 조회는 미룹니다. 로컬 토큰 집계는 iCloud 동기화가 켜져 있으면 계속하며, 꺼져 있으면 화면을 열 때까지 미룹니다. 토큰 화면을 다시 열면 오래된 결과를 갱신하며 기존 로그 캐시를 재사용합니다. 표시용 시계는 창이 보일 때만 분 단위로 동작합니다. 네트워크 실패 시 마지막 성공 값을 유지하며 `이전 데이터`로 표시합니다. 새로 실행했을 때는 캐시 없이 실제 데이터를 요청합니다.
...
dist/Co-Count.app/Contents/MacOS/Co-Count --snapshot /tmp/cocount-trend.png --trend # 감소 그래프 미리 보기
...
- 어제 토큰과 7일 그래프는 **서버 일별 집계**이며 지연될 수 있습니다. 오늘 숫자는 **동기화된 Mac 기록의 통합 추정치**를 표시합니다. iCloud 동기화를 끄면 이 Mac의 기록만 표시합니다. 서버 미집계는 `—`로 표시하고, 로컬 기록을 정상 조회했지만 오늘 활동이 없으면 추정 0입니다.
...
기존 토큰 카드의 크기, 오늘·어제 배치, 최근 7일 막대, 하단 시간대 그래프를 유지하면서 수치만 한도 사용률(%)로 전환합니다. 기본 표시는 **한도 사용**입니다. 카드 상단의 **사용된 토큰 / 한도**의 작은 선택 버튼 또는 **설정 → 카드 기본 표시**에서 표시 방식을 바꾸면 선택이 저장됩니다. 두 선택지를 항상 표시하고, `사용된`은 일반 텍스트로 두고, 선택된 단위 버튼만 배경색으로 강조합니다. 우측 **주간 기준**을 누르면 서버가 제공하는 다른 한도(예: 5시간)로 전환됩니다. 두 한도의 비율은 합치지 않습니다.
...
- 두 그래프는 7초마다 막대와 감소 꺾은선을 1.2초 페이드로 함께 전환합니다. 7일 꺾은선은 100%에서 기록된 사용량을 누적 차감하고, 하루 꺾은선은 00시 잔여 한도에서 현재 시각까지 기록분을 차감합니다. 하루의 세로 범위는 작은 변화도 보이도록 확대하며 시작·마지막 값을 표시합니다. 날짜 선택은 두 화면 모두 지원합니다.
- 자정 기록이 없으면 첫 기록 시각부터 시작하며, 기준값이 없는 예전 요약은 `00시 한도 기록 없음`으로 표시합니다. 새 일별 요약에는 시작 기준값도 보관합니다. 점선은 관측 부족 구간이며 리셋으로 보충된 한도는 더하지 않고 0%까지만 표시합니다. 실제 잔여 한도는 상단 게이지로 확인할 수 있습니다. 화면이 모두 숨겨지면 전환 타이머도 멈추고, 동작 줄이기 설정에서는 페이드를 생략합니다.
...
## 두 Mac의 iCloud 사용량 동기화

기본으로 켜져 있습니다. 두 Mac에서 같은 Apple 계정으로 iCloud Drive를 켜고, 같은 Codex 계정·플랜으로 이 버전의 Co-Count를 실행하세요. 설정의 **iCloud 사용량 동기화**에서 기기별 마지막 기록 시각과 **iCloud 동기화 폴더 열기**를 확인할 수 있습니다. 앱이 실행 중이면 화면이 닫혀 있어도 설정한 갱신 간격(기본 5분)으로 수집합니다. iCloud 전송 자체는 macOS가 관리하므로 두 기기의 화면이 즉시 같아지는 것은 보장하지 않습니다.

전용 폴더는 `~/Library/Mobile Documents/com~apple~CloudDocs/Co-Count/UsageSync/v1`입니다. 앱 전용 폴더이므로 Codex의 설정·인증 파일·대화 원본을 옮기거나 공유할 필요가 없습니다. iCloud Drive가 꺼져 있으면 그 자리에 일반 폴더를 만들지 않으며, 로컬 보관함에 기록을 쌓아 다음 정상 연결에 재전송합니다.

```text
Co-Count/UsageSync/v1/
  <계정·플랜 해시>/
    <맥북 기기 해시>/
      <UTC 날짜 번호>.json
    <맥미니 기기 해시>/
...
- 각 기기는 자기 파일만 씁니다. 공유 인덱스·공유 합계 파일이 없으며, 받은 데이터를 다시 내보내지 않습니다. 다른 Mac의 파일을 수정하거나 정리하지 않습니다.
- 기기 ID는 로컬 설치 UUID와 하드웨어 식별자의 해시로 구분합니다. 홈 폴더를 다른 Mac으로 복원해도 별도 작성자가 되며, 하드웨어 식별자 원문은 공유하지 않습니다. 같은 Mac에서 실행 파일을 중복 실행하면 로컬 잠금으로 쓰기를 직렬화합니다.
- 계정 동기화 키에는 Mac마다 다른 `CODEX_HOME` 경로를 넣지 않습니다. 기존 로컬 이력 키는 유지해 이전 기록을 보존합니다. 계정 식별에 실패하면 공유 파일을 쓰지 않습니다.
- 파일에는 기기 이름, 관측 시각·한도·리셋 시각, 토큰 이벤트 해시·시각·증가분, 초기 이전용 과거 요약만 저장합니다. 이메일·대화·인증 정보는 저장하지 않습니다. 로컬 JSONL에는 계정 식별자가 없으므로 한 Codex 홈에서 여러 계정을 번갈아 쓴 기록은 정확히 분리할 수 없습니다.
- 토큰은 동일 이벤트 해시를 한 번만 집계합니다. 오늘 및 전날 꼬리를 함께 스캔해 자정 직전의 미수집 이벤트를 보완하고, 표시할 때는 보는 Mac의 오늘 날짜만 합산합니다. 이전 날짜 토큰은 기존 서버 집계를 사용합니다.
- 한도는 두 기기의 관측을 시각순으로 합친 뒤 감소분을 계산합니다. 같은 리셋 기간에 지연된 응답으로 잔여율이 다시 높아져도 이전 감소분을 중복 계산하지 않습니다. 30분 초과 공백·리셋 경계는 연결하지 않습니다. 사용률 합계끼리 더하지 않습니다.
- 기존 15분 요약은 초기 이전에 공유합니다. 상세 관측이 있는 구간을 우선하고, 요약끼리 겹치면 관측 범위가 더 충실한 쪽을 사용합니다. 요약만 남은 과거의 정확한 관측 구간을 복원할 수는 없으므로 겹침 처리는 추정입니다.
- 전용 로컬 보관함 `~/Library/Application Support/Co-Count/UsageSync/v1`에 먼저 원자적으로 저장한 뒤 iCloud에 반영합니다. 모든 공유 파일 접근은 `NSFileCoordinator`를 사용하고, 종료·설정 변경 시 진행 중인 조정을 취소합니다. 동기화 오류가 나도 서버 한도 표시와 기존 로컬 이력은 유지됩니다.
- 파일 수정 시각·크기·파일 ID가 같으면 내용을 다시 읽지 않습니다. 내용이 같으면 재인코딩·쓰기를 생략합니다. 관측·스캔 시각의 갱신은 최소 1분 간격으로 묶고, 변하지 않는 한도 구간은 시작·끝만 보관합니다. 정상적인 정기 관측은 관측 범위와 기기 활동 시각을 갱신하므로 새 현재 날짜 파일을 기록할 수 있습니다.
- 오늘 UTC 파일과 이전 9일만 조회합니다. 하루 한 번 자신의 만료 파일만 정리합니다. 다른 기기의 만료 파일은 그 기기가 다시 실행될 때 정리합니다. iCloud에서 아직 내려받지 않은 파일은 다운로드를 요청하고 기다리며, 요청 재시도는 5분 간격으로 제한합니다.
- 손상되거나 일부만 도착한 파일은 반영하지 않고 메모리의 마지막 정상 기록을 유지합니다. 같은 손상 파일을 반복 파싱하지 않으며 내용이 바뀌면 다시 확인합니다. 30분 이상 갱신되지 않은 다른 기기는 지연 상태로 표시합니다. 양쪽 모두 앱이 꺼져 있던 시간의 한도 변화는 복구할 수 없습니다.

동기화를 끄면 기록 교환과 백그라운드 토큰 수집을 중단하고 이 Mac의 이력을 표시합니다. 기존 공유 기록을 삭제하지 않으므로 다시 켜면 이어서 통합합니다. 구현과 검증은 [동기화 설계](docs/sync.md)에 정리했습니다.
