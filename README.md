...
기본 갱신 간격은 5분입니다. 패널을 열 때 데이터가 1분 이상 지났으면 갱신합니다. 창이 보이지 않으면 한도 조회와 이력 저장만 계속하고 토큰 조회·로컬 집계는 미룹니다. 토큰 화면을 다시 열면 오래된 결과를 갱신하며 기존 로그 캐시를 재사용합니다. 표시용 시계는 창이 보일 때만 분 단위로 동작합니다. 네트워크 실패 시 마지막 성공 값을 유지하며 `이전 데이터`로 표시합니다. 새로 실행했을 때는 캐시 없이 실제 데이터를 요청합니다.
